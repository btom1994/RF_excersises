...
\begin{enumerate}
	...
	\]
	Diese ist eine lange exakte Kohomologiesequenz\footnote{Das ist eine sehr bekannte Tatsache, welche wir an dieser Stelle nicht zeigen werden.}, wobei die Garbe $\underline{\mathbb{Z}}$ jeweils die passende ist, z.B für $\check H^{q}(U ,\underline{\mathbb Z})$ ist dies die Garbe welche auf $U$ die konstante Garbe ist und auf dem Komplement von $U$ 0 ist. Die Kohomologie verhält sich aber wie gewohnt.
	Außerdem gilt $\check{H}^r(U,\underline{\mathbb{Z}})=0$ für $r\geq1$ und $U\cong\mathbb{C}$\footnote{Das wissen wir bisher nur für $r=1$. Da wir nie mit $\check{H}^2$ gearbeitet haben, sei diese Aussage auch ohne Beweis gegeben.}.
\end{enumerate}
