...
\Aufg
\begin{enumerate}
	\item Beweise das folgende Lemma.
	\begin{lemma}
	...
	\emph{Hinweis:} \(f(x+iy)=\int_0^1 \frac{d}{dt} f(tx+ity)\,dt=\ldots\)
	
	\item  Sei $X$ eine Riemannsche Fläche und $z:U\to U'$ eine Karte um $p$. Zeige, dass für das maximale Ideal $m_p\lhd\mathcal{E}_p$ der bei $p\in X$ verschwindenden $C^\infty$-Funktionen gilt:
	\[
...
\]
Zeige, dass man in folgender Weise ein $f^*$ für holomorphe $1$-Formen hat: Ist $\omega\in \Omega^1(V)$ und $z:W\to W'$ eine Karte für $Y$ mit $W\subset V$, so schreibe $\omega|_W=\varphi(z)\,dz$ und setze
\[
