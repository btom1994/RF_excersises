...
\medskip \centerline{7. Blatt -- Abgabe 03.12, Übung 04.12}
...
\]
(Beachte, dass nur endlich viele Summanden ungleich Null sind.)
...
\begin{enumerate}
	\item Ist \( X \) kompakte Riemannsche Fläche vom Geschlecht \( g \) mit beliebiger Triangulierung, so gilt für die Eulercharakteristik:
	\[
	\chi(X) = 2 - 2g.
	\]	
	Damit ist $\chi(X)$ insbesondere unabhängig von der gewählten Triangulierung auf $X$.
	\item In obiger Situation gilt:
	\[
	\chi(X) = m \cdot \chi(Y) - b(f).
	\]
	
	\emph{Hinweis:} Aufgabe 21
	\item Folgere nun die Riemann-Hurwitz-Formel.
\end{enumerate}

\medskip

\Aufg Zeige mithilfe der Riemann-Hurwitz-Formel und der Weierstraß $\wp$-Funktion, dass ein Torus Geschlecht 1 hat.

\medskip

\Aufg Im Folgenden seien $X,Y$ kompakte Riemannsche Flächen und $f$ stets holomorph und nicht konstant. Leite aus der Riemann–Hurwitz-Formel her:

\begin{enumerate}
	\item Wenn \( f : X \to Y \) existiert, dann gilt:
	\(
		g(X) \ge g(Y)
	\) 
	\item Wenn \( f : \mathbb{CP}^1 \to Y \) existiert, dann ist \( Y \)
	homöomorph zur Sphäre.
	\item Wenn \( f : X \to Y \) mit Blätterzahl 1 existiert, so ist $f$ schon biholomorph.
	\item Wenn $f\in \mathcal{M}(X)$ mit nur einem einfachen Pol exisitiert, so ist $X$ isomorph zu $\mathbb{CP}^1$.
\end{enumerate}
