...
\Aufg Seien $\mathcal{F}$ und $\mathcal{G}$ Garben auf dem topologischen Raum $X$ und 
$\alpha : \mathcal{F} \to \mathcal{G}$ ein Garbenmorphismus. Zeigen Sie, dass die Definition
\[
\ker(\alpha) : U \mapsto \ker \bigl(\alpha(U) : \mathcal{F}(U) \to \mathcal{G}(U)\bigr)
\]
eine Garbe liefert, die wir mit $\ker \alpha$ bezeichnen.

\medskip

\Aufg Vervollständigen Sie das „Nicht-Beispiel 6“ der Vorlesung: Für $X = \mathbb{C}$ ist die Prägarbe
\[
...
\Aufg Sei $X$ eine Riemannsche Fläche. Zeigen Sie, dass
\[
\underline{2\pi i \mathbb{Z} }\;=\; \ker \Bigl(\exp : \mathcal{O}_X \to \mathcal{O}_X^\times \Bigr)
\]
gilt, wobei $\exp$ der durch $\exp(f) = e^f$ induzierte Garbenmorphismus sei und $\underline{2\pi i \mathbb{Z}}$ die konstante Garbe zur abelschen Gruppe $2\pi i \mathbb{Z}$ ist.

\medskip


\Aufg Sei $X$ ein topologischer Raum und $A$ eine abelsche Gruppe. Zeigen Sie, dass für $x \in X$
\[
\underline{A}_x = A
\]
kanonisch gilt.
