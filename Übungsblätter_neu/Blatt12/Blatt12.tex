...
\Aufg Sei $X$ ein topologischer Raum mit einer offenen Überdeckung $\mathscr{U}=(U_i)_{i\in I}$ gegeben. Weiter sei zu jedem $i\in I$ eine Garbe $\mathcal{F}_i$ auf $U_i$ gegeben. Außerdem haben wir Isomorphismen $\Phi_{ij}:\mathcal{F}_i|_{U_{ij}}\to\mathcal{F}_j|_{U_{ij}}$ auf $U_{ij}=U_i\cap U_j$ gegeben, welche $\Phi_{ii}=\text{id}_{\mathcal{F}_i}$ und $\Phi_{ik}=\Phi_{jk}\circ\Phi_{ij}$ auf $U_{ijk}$ erfüllen. Zeige, dass wir die $\mathcal{F}_i$ zu einer Garbe $\mathcal{F}$ auf $X$ verkleben können. Genauer: Zeige, dass dann eine Garbe $\mathcal{F}$ auf $X$ mit $\mathcal{F}|_{U_i}\cong\mathcal{F}_i$ für alle $i\in I$ existiert. 
