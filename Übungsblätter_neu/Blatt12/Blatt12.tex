...
\begin{enumerate}[label=\roman*)]
	\item \(\eta=(g_{ij})_{ij}\in \check{C}^1(\mathscr{U},\mathcal{O}^\times)\) gilt, und wir somit eine Klasse
	\[
...
\Aufg Ist umgekehrt \(\mathscr{U}\) eine offene Überdeckung und ein Kozykel \((g_{ij})_{ij}\in \check{C}^1(\mathscr{U},\mathcal{O}^\times)\) gegeben, dann konstruiere dazu ein Geradenbündel \(\mathcal{L}\), so dass \(c(\mathcal{L})=[(g_{ij})_{ij}]\in\check{H}^1(X,\mathcal{O}^\times)\) gilt.

\medskip

\textbf{Bemerkung:} Wir haben nun Teile des Satzes bewiesen, dass man einen Isomorphismus
\[
