...
\Aufg Wir wollen in dieser Aufgabe die Behauptung 1 im Beweis der Existenz der universelle  Überlagerung zeigen. Sei also $Y$ eine Mannigfaltigkeit und $y_0 \in Y$. Ferner sei $\alpha$ ein Weg von $y_0$ nach $y \in Y$. Für eine einfach zusammenhängende, offene Umgebung $U$ von $y$ sei
\[
...
\begin{enumerate}
	...
	\]
	kommutiert. Untersuche dazu die geometrischen Eigenschaften der eigentlichen Abbildung $\wp$: Wo ist sie verzweigt? Wieso sind die Punkte $b_1 :=\wp( \tfrac{1}{2})$, $b_2 := \wp(\tfrac{\tau}{2})$ und $b_3 := \wp(\tfrac{1+\tau}{2})$ und $b_4 := \infty$ besonders? Welche Überlagerung ist $\wp$ außerhalb der verzweigten Punkte im Sinne von: welche Fasern hat diese Überlagerung und was hat das damit zu tun, dass $\wp$ gerade ist? Was nutzt es dann, dass $f$ auch gerade sein soll?
